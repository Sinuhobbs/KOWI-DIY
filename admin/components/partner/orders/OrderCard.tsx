...
import { formatReadyIn, orderNumber } from "@/lib/partner/orders";
...
function statusCopy(order: ActiveOrder) {
  if (order.status === "NEW") return "NEW";
  if (order.status === "PREPARING") return "PREPARING";
  return "DISPATCHED";
}

function statusClass(order: ActiveOrder) {
...
function ctaLabel(order: ActiveOrder) {
  if (order.status === "NEW") return "Accept Order";
  if (order.status === "PREPARING") {
    const left = order.readyInSec ?? 0;
    return left > 0 ? `Mark Ready in ${formatReadyIn(left)}` : "Mark Ready";
  }
...
export function OrderCard({
  ...
  return (
    <article className="rounded-[20px] border border-kowi-line bg-white p-3.5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] font-bold leading-4 ${statusClass(order)}`}
          >
            {statusCopy(order)}
          </span>
        </div>
      ...
      <div className="mt-2.5 flex items-center gap-2 text-[13px] font-bold text-kowi-ink">
      ...
      <div className="mt-3 grid grid-cols-4 gap-2">
        ...
        {showViewAll ? (
          <Link
            ...
            className="flex aspect-square w-full flex-col items-center justify-center self-start rounded-[12px] bg-[#f4f5f7] px-1 text-center"
          >
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 border-t border-dashed border-kowi-line pt-3">
        <a
          href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(order.area)}`}
          target="_blank"
        ...
        >
          ...
          <span className="relative text-center">{ctaLabel(order)}</span>
        </button>
      </div>
    </article>
  );
